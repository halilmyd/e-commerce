...
const DetailCard = ({ urunAcıklaması, urunAdı,FavoriesNotifyRemove,removeInFavories, urunIndırımOranı, id, notifySucces, urunIndirimliFiyat, basketNotifySucces, urunIndirimsizFiyat, urunResmi, addToFavories, addBasket, removeInBasket, BasketNotifyRemove }) => {
    ...
    const [urunFavorideMi, setUrunFavorideMi] = useState(false)
    ...
    useEffect(() => { 
        ...
        if(sepet.includes(params.id)){
            setUrunSepetteMi(true)
        }

        const favoriler = (JSON.parse(localStorage.getItem("favoriler")))
        console.log("favoriler",favoriler)
        if(favoriler.includes(params.id)){
            setUrunFavorideMi(true)
        }
            ...
            <div className="en-dıs container">
                ...
                <div className="özellik">
                    ...
                    <div className="urun-buton">
                        {!urunSepetteMi && <a onClick={basketNotifySucces}><button onClick={() => addBasket(id)} className="ui primary button butons ">Sepete Ekle</button></a>}
                        {urunSepetteMi && <a onClick={BasketNotifyRemove}><button onClick={() => removeInBasket(id)} className="ui primary button butons ">Sepetten Çıkar</button></a>}
                       { !urunFavorideMi &&<a onClick={notifySucces}><i className="heart outline icon kalps" onClick={() => addToFavories(id)}></i></a>}
                        {urunFavorideMi &&<a onClick={FavoriesNotifyRemove}><i className="heart icon kalps"  onClick={() => removeInFavories(id)}></i></a>}
                    </div>
                </div>
            </div>
